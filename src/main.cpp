...
double a_funct(double x) { return x * x; }
...
int main() {
  ...
  cout << square_root(val) << endl
       << endl;
  cout << "The integral of (x*x) from 2.0 to 3.0 using 1000 intervals is: ";
  cout << integral(2.0, 3.0, 1000, *a_funct) << endl
       << endl;
  ...
  int x = 5;
  int y = 5;
  cout << "The number of paths (using only up and right moves) through your "
          "grid\t";
  cout << paths(x, y) << endl;
  return 0;
}
